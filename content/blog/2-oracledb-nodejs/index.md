...
We will show how to construct REST APIs serializing via JSON with Node.js and perform majority of the backend work in the Oracle database, how to store images in a binary format to avoid hassle with pictures on filesystem and tem- porary images and we will provide general tips and overview of Oracle SQL Developer. If you are considering using Oracle database, or actually are in posi- tion that you have to, this document might be an inspiration or quick overview of some experiences and crafted working solutions that might be useful or you might use.
...
- node-oracledb 5.3.0,
- features facilitating the development and working.
...
Noteworthy item here is the connectionString which must be included to spe- cify connection creds. Then we can execute queries thanks to classes as follows:
```javascript
...
We will outline several types of endpoints which are needed not only in our applicaton but in general. Some of ours, however, utilise serialization of SE- LECT via json_object in Oracle Database, some send picture from assembled bytestream from database BLOB, some server for insertions, etc.
...
```
Selected image get streamed with header to client via backend as a HTTP re- sponse. Preview of parametrized query is possible to be viewed in SQL Developer even with "image representation of bytestream"- which in this case is a JPEG image.
<p align="center">
